...
namespace MaxyClass
{
    public class Maxyber
    {
        ...
        // ищет максимальное число среди массива чисел
        static public int maxNumber(int[] nums)
        {
            int max = nums[0];
        ...
        // метод генерации массива, состоящего из целых чисел в интервале от firstNum до lastNum в количестве count элементов
        static public int[] RandArray(int firstNum, int lastNum, int count)
        {
            int[] resArray = new int[count];
            Random rand = new Random();
            for (int i = 0; i < count; i++)
            {
                resArray[i] = rand.Next(firstNum, lastNum);
            }
            return resArray;
        }
